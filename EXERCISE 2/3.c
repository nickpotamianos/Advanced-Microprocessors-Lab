...
// ISR for the switch press to toggle the inverse mode
ISR(PORTF_PORT_vect) {
    if (PORTF.INTFLAGS & PIN5_bm) { // Check if switch interrupt flag is set
        inverse = !inverse; // Toggle inverse flag
        end = 0; // Reset end count when direction is toggled
